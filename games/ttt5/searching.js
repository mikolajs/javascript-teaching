      ...
      if (this.readyState == 4 && this.status == 200) {
         ...
         if(objJSON.add == "OK") {
             window.location.href = window.location.protocol +
              "//" + window.location.host + "/ttt5/game.html?room=" + room;
         } else if(objJSON.error == 'already in'){
             window.location.href = window.location.protocol +
              "//" + window.location.host + "/ttt5/game.html?room=" + room;
         } else self._createPopInfoModal('Błąd', 'Poszukiwany pokój gry już nie istnieje');
...
///// TODO: go to empty room
  makeOwnGame(){
    ...
    this.xhttpCon.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        let objJSON = JSON.parse(this.responseText);
        if(objJSON.add == "OK") {
          window.location.href = window.location.protocol + "//" + window.location.host + "/ttt5/game.html?room=" +
          objJSON.roomCode;
        } else {
  ...
  _closeCard(){
     window.location.href = window.location.protocol + "//" + window.location.host + "/ttt5/login.html";
  }
    ...
    this.xhttp.onreadystatechange = function() {
     ...
     console.log("Resp for load Players: " + this.responseText);
       ...
       for(let i = 0; i < aPlayers.length; i++){
         player = aPlayers[i];
         //console.log("c1: " + player.code + " & c2: " + this.cookie);
         if(player.code == self.cookie){
           document.getElementById('playerName').innerHTML = player.name;
           continue;
         }
         let strHTML = '<tr><td>' + player.name + '</td></tr>';
       ...
       for(let i = 0; i < rooms.length; i++){
         let strHTML = '<tr><td>' + rooms[i].player1.name + '</td><td>';
           if(rooms[i].player1.code == self.cookie || rooms[i].player2.code == self.cookie){
            strHTML += '<button class="btn btn-info" onclick="searching.connectGame(\'';
            strHTML += rooms[i].roomCode +  '\')">Wróć do pokoju</button></td></tr>';
            tableRoomBody.innerHTML += strHTML;
          } else  if(rooms[i].player2.name != '' && rooms[i].player1.name != '') {
            continue;
         } else {
           strHTML += '<button class="btn btn-info" onclick="searching.connectGame(\'';
